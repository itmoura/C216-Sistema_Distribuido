...
public class LoadingDemo {
	...
	public void demoEagerLoading() {
		try{
			var item = service.buscarNotaCompraItemPeloId(1L);
			var dataEmissao = item.getNotaCompra().getDataEmissao();
	...
	public void demoLazyLoading() {
		try {
			var nota = service.buscarNotaCompraPeloId(1L);
	...
	public void demoPlanejandoConsulta() {
		try {
			var nota = service.buscarNotaCompraPeloIdComListaItem(1L);
			int tamanho = nota.getListaNotaCompraItem().size();
			System.out.println(tamanho);
